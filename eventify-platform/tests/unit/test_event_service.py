...
# Ensure event-service path is first in sys.path
event_service_path = os.path.join(os.path.dirname(__file__), '..', '..', 'services', 'event-service')
if event_service_path not in sys.path:
    sys.path.insert(0, event_service_path)

# Clean up any conflicting app modules from other services
for key in list(sys.modules.keys()):
    if key.startswith('services') or (key == 'app' and 'notification' in str(sys.modules.get(key, ''))):
        del sys.modules[key]
