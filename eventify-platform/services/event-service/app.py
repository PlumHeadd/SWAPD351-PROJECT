...
from metrics import (
    track_request, track_event_operation, track_rsvp_operation,
    track_cache_operation, track_rabbitmq_publish,
    record_cache_hit, record_cache_miss, record_event_created, record_rsvp_created,
...
def publish_message(routing_key, data):
    try:
        with track_rabbitmq_publish(routing_key):
            ...
            if ch:
                ...
                print(f"📨 Published message: {routing_key}")
            else:
                print(f"⚠️  RabbitMQ channel not available for: {routing_key}")
    except Exception as e:
        print(f"❌ Error publishing message {routing_key}: {e}")
...
@app.before_request
def before_request():
    """Track request start and active requests"""
    request._start_time = datetime.utcnow()
    http_active_requests.labels(method=request.method).inc()
...
def after_request(response):
    ...
    if hasattr(request, '_start_time'):
        ...
        track_request(request.method, request.path, response.status_code, duration)
    http_active_requests.labels(method=request.method).dec()
    return response
...
@track_event_operation('create_event')
def create_event():
    user_id = get_user_id()
    if not user_id:
...
@track_rsvp_operation('create_rsvp')
